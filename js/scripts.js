...
function fetchMapData(query, optionsBodyArg = {}) {
...
class Input {
    ...
    highlitMatches(fullMatch = null) {
        ...
        if (this.sortHighlightedMatches) this.sortHighlightedMatches();

        function renderHighlight(valText, value) {
            const valTextMod = valText.toLowerCase().trim();
            const substrPos = valTextMod.indexOf(value);
            if (substrPos < 0) return valText;
    ...
    sortHighlightedMatches(key = "node") {
        // для TextInputCheckboxes key === "textNode". Метод вызывается одноименном дочернем методе класса TextInput или TextInputCheckboxes
        sortCallback = sortCallback.bind(this);
        const sortedValues = this.selectValues.sort(sortCallback).map(val => val[key].innerHTML);

        // запомнить те подсказки, которые не имеют совпадений с введенной строкой
        let hiddenValues;
        if (this.params.showUnmatched !== "true")
            hiddenValues = this.selectValues.filter(selVal => selVal.node.classList.contains("none"));

        // выставить все подсказки
        ...
        // убрать те подсказки, которые не имеют совпадений с введенной строкой
        if (this.params.showUnmatched !== "true") {
            this.selectValues.forEach(selVal => {
                const similar = hiddenValues.find(v => v.text === selVal.text);
                if (!similar) return;
                selVal.node.classList.add("none");
            });
        }

        function sortCallback(val1, val2) {
            const highlight1 = val1.node.querySelector(".highlight");
            const highlight2 = val2.node.querySelector(".highlight");
            const highlight1Content = highlight1 ? highlight1.textContent : "";
            const highlight2Content = highlight2 ? highlight2.textContent : "";

            const val1first = !highlight2 || highlight1Content.length > highlight2Content.length;
            const val2first = !highlight1 || highlight1Content.length < highlight2Content.length;
            if (val1first) return -1;
            if (val2first) return 1;
            return 0;
        };
    }
    checkCompletion() {
    ...
    toggleCompletionBg() {
        ...
        function setBg() {
            if (!this.completionBg) {
                ...
                setTimeout(() => this.inputWrapper.append(this.completionBg), 0);
            }
            setTimeout(() => this.completionBg.classList.remove("none"), 0);
        }
        function removeBg() {
...
class TextInput extends Input {
    ...
    editSelectValues(params = {}) {
        ...
        */
        ...
        this.getSelectsWrap();
        if (!params.noHighlightCall) this.highlitMatches();
    ...
    sortHighlightedMatches() {
        super.sortHighlightedMatches("node");
    }
    addTag() {
...
class TextInputCheckboxes extends Input {
    ...
    editSelectValues(params = {}) {
        ...
        function createItem(value) {
            ...
            <label class="flex checkboxes__items_item">
                <input class="mr-5 checkbox selects-checkbox" type="checkbox" value="${value.replace(/<.*?>/g, "")}">
    ...
    sortHighlightedMatches() {
        super.sortHighlightedMatches("textNode");
    }
    addTags() {
...
class MapBlock {
    ...
    initSearchInputs() {
            ...
            function onMutation() {
                setTimeout(() => {
                    ...
                    for (let key in inputsSearchParams) {
                        ...
                        if (obj.handler) inputParams.input.addEventListener("input", obj.handler.bind(this));
        ...
        function handlePrompt(query, params = {}) {
            /*
                ...
                optionsBodyArg - объект, передаваемый в body при послании запроса к api dadata
            ...
            */
            return new Promise(resolve => {
                this.getPrompt(query, params.optionsBodyArg)
                    .then(json => {
        ...
        function setCityDistrictPrompt() {
            ...
            const cityValue = this.citySearchParams.input.value.replace(/\(.*\)/g, "");
            if (!cityValue) return;

            const query = `${cityValue} р-н`;
            handlePrompt(query, {
    ...
    getPrompt(query, optionsBodyArg) {
        return new Promise((resolve, reject) => {
            fetchMapData(query, optionsBodyArg)
    ...
    getAddr() {
        ...
        if (city && street && house) this.getCityDistrict(addrValue);
    ...
    getCityDistrict(addrValue) {
        if (!this.cityDistrictSearchParams) return;
        if (!this.cityDistrictSearchParams.input.closest("body")) return;

        this.getPrompt(addrValue, { division: "administrative" })
            .then(json => {
                const cityDistrictData = json.suggestions.find(d => d.unrestricted_value.includes("р-н"));
                if (!cityDistrictData) return;

                const cityDistrictValue =
                    cityDistrictData.unrestricted_value.split(", ").find(val => val.includes("р-н"));
                this.cityDistrictSearchParams.input.value = cityDistrictValue.replace("р-н", "").trim();
                this.cityDistrictSearchParams.input.dispatchEvent(new Event("input"));
                this.cityDistrictSearchParams.input.dispatchEvent(new Event("change"));
            });
    }
}
